            ...
            {failureReasons.map((reason) => (
              <Card key={reason.title} className="p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="font-display text-base font-semibold text-heading">{reason.title}</h3>
                </div>
              ...
              >
                <Card variant="glass" hover={false} className={`p-6 ${idx % 2 !== 0 ? "lg:order-2" : ""}`}>
                  ...
                  <div className="rounded-lg bg-white/5 p-3">
                    <p className="text-xs text-muted uppercase tracking-widest mb-1">Best For</p>
                    <p className="text-sm text-body">{model.useCases}</p>
                  </div>
                </Card>

                <div className={idx % 2 !== 0 ? "lg:order-1" : ""}>
                  <h3 className="font-display text-2xl font-bold text-heading mb-4">{model.title}</h3>
                  <p className="text-body leading-relaxed">{model.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
            {capabilities.map((cap) => (
              <Card key={cap.title} variant="service" className="p-6">
                ...
                <h3 className="font-display text-lg font-semibold text-heading mb-2">{cap.title}</h3>
