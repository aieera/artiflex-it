...
import { secOpsDisciplines } from "../data/secOpsVendors";
...
export default function SecOpsVendorDetail() {
  const { discipline, slug } = useParams<{ discipline: string; slug: string }>();
  const group = discipline ? secOpsDisciplines[discipline] : undefined;
  const vendor = group && slug ? group.vendors[slug] : undefined;
  ...
  if (!group || !vendor) {
    return <Navigate to="/cybersecurity/security-operations" replace />;
  }

  const metaDescription = (() => {
    const raw = vendor.description.replace(/\s+/g, " ").trim();
  ...
  const pageUrl = `https://artiflexit.com/cybersecurity/security-operations/${discipline}/${vendor.slug}`;
  const pageTitle = `${vendor.name} | ${group.label} | Security Operations UAE | Artiflex IT`;
  const ogImage = "https://artiflexit.com/og/cybersecurity.png";
  ...
  return (
    <>
      ...
      {/* hreflang, UAE primary, Oman + Saudi Arabia secondary */}
      <link rel="alternate" hrefLang="en-ae" href={pageUrl} />
      <link rel="alternate" hrefLang="en-om" href={pageUrl} />
      <link rel="alternate" hrefLang="en-sa" href={pageUrl} />
      ...
      <script type="application/ld+json">
        {JSON.stringify({
          ...
          "@type": "Product",
          name: vendor.name,
          category: `${group.label} (Security Operations)`,
      ...
      <script type="application/ld+json">
        {JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 3, name: "Security Operations", item: "https://artiflexit.com/cybersecurity/security-operations" },
            { "@type": "ListItem", position: 4, name: group.label, item: `https://artiflexit.com${group.href}` },
            { "@type": "ListItem", position: 5, name: vendor.name, item: pageUrl },
          ],
        })}
      </script>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#04101E] to-[#0A3D6B]">
        ...
        <div className="relative z-10 border-b border-white/5">
          <div className="shell flex w-full flex-wrap items-center gap-2 pt-24 pb-3 text-[11px] text-slate-400 sm:pt-28">
            <Link to="/" className="transition-colors hover:text-white">Home</Link>
            <span className="text-slate-600">/</span>
            <Link to="/cybersecurity" className="transition-colors hover:text-white">Cybersecurity</Link>
            <span className="text-slate-600">/</span>
            <Link to="/cybersecurity/security-operations" className="transition-colors hover:text-white">Security Operations</Link>
            <span className="text-slate-600">/</span>
            <Link to={group.href} className="transition-colors hover:text-white">{group.label}</Link>
            <span className="text-slate-600">/</span>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-16">
            ...
            >
              ...
              <div className="mt-10 flex flex-col gap-5">
                <div className="flex flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-3">
                  ...
                  <Link
                    to={`${group.href}#vendor-comparison`}
                  ...
                  <Link
                    to={`${group.href}#gartner-comparison`}
                    className="group inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-xs font-semibold text-slate-200 transition-all hover:-translate-y-0.5 hover:border-[#28B5E1]/60 hover:bg-white/10 hover:text-white sm:text-[13px]"
                  >
            ...
            >
              <div className="relative mx-auto flex aspect-[4/3] max-w-md items-center justify-center overflow-hidden rounded-3xl border border-white/15 bg-white p-10 shadow-[0_30px_80px_-20px_rgba(3,16,28,0.8)] ring-1 ring-[#28B5E1]/10 sm:p-14">
                ...
                <img src={vendor.logo} alt={`${vendor.name} logo`} className="relative z-10 max-h-32 w-full max-w-[80%] object-contain" />
              </div>
            </motion.aside>
          </div>
        </div>
      ...
      {vendor.keyStats.length > 0 && (
        <section className="relative bg-slate-50 py-12 sm:py-16">
          <div className="mx-auto max-w-4xl px-8 sm:px-6">
            <div className={`grid gap-4 sm:grid-cols-2 ${vendor.keyStats.some((s) => s.wide) ? "lg:grid-cols-3" : "lg:grid-cols-4"}`}>
              {vendor.keyStats.map((s) => (
                <div key={s.label} className={`rounded-2xl border border-slate-200 bg-white p-5 shadow-sm ${s.wide ? "sm:col-span-2 lg:col-span-3" : ""}`}>
                  <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.22em] text-[#1B8AC7]">{s.label}</p>
                  <p className="mt-2 font-display text-base font-semibold text-slate-900 sm:text-lg">{s.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="shell relative">
            <div className="mx-auto max-w-3xl text-center">
              ...
              <h2 className="mt-6 font-display text-3xl font-bold leading-[1.05] tracking-tight text-slate-900 sm:text-4xl md:text-5xl">
                ...
                <span className="bg-gradient-to-r from-[#1B8AC7] via-[#28B5E1] to-[#9CD6EE] bg-clip-text text-transparent">{vendor.whatIs.titleHighlight}</span>
              </h2>
            </div>
              ...
              <div className="group relative overflow-hidden rounded-3xl border border-white/15 bg-gradient-to-br from-[#0A3D6B] via-[#04101E] to-[#020617] p-7 shadow-[0_20px_60px_-15px_rgba(15,28,46,0.45)] transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_30px_70px_-15px_rgba(40,181,225,0.45)] sm:p-9">
                ...
                <div className="relative">
                  <h3 className="font-display text-2xl font-bold leading-[1.1] text-white sm:text-3xl md:text-[2rem]">
                    ...
                    <br />
                    <span className="bg-gradient-to-r from-[#28B5E1] to-[#9CD6EE] bg-clip-text text-transparent">{vendor.whatIs.feature.titleLine2}</span>
                  </h3>
                  <p className="mt-5 text-sm leading-[1.7] text-white/80 sm:text-[15px]">{vendor.whatIs.feature.body}</p>
                </div>
              </div>
            </div>
      ...
      {/* WHY IT WINS */}
      <section id="review" className="relative overflow-hidden bg-slate-50 py-16 sm:py-24">
        ...
        <div className="shell relative">
          <div className="mx-auto max-w-3xl text-center">
            ...
            {vendor.whyWinsIntro?.description && (
              <p className="mt-5 text-base leading-relaxed text-slate-600 sm:text-lg">{vendor.whyWinsIntro.description}</p>
            )}
          </div>
            ...
            {vendor.whyWinsIntro?.stats && vendor.whyWinsIntro.stats.length > 0 && (
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                {vendor.whyWinsIntro.stats.map((stat, i) => {
                  const toneText = { emerald: "text-emerald-300", violet: "text-violet-300", sky: "text-sky-300" }[stat.tone];
                  return (
                    <motion.div
                      key={stat.label}
                    ...
                    >
                      <p className={`font-display text-2xl font-bold leading-none sm:text-2xl ${toneText}`}>{stat.value}</p>
          ...
          <div className="mt-12 grid items-start gap-1 lg:grid-cols-2">
            {vendor.strengths.map((s, i) => {
              ...
              const isOrphan = i === vendor.strengths.length - 1 && vendor.strengths.length % 2 === 1;
              return (
                <motion.div
                  key={s.title}
                  tabIndex={0}
                ...
                >
                  <div className="flex items-stretch gap-2.5">
                    ...
                    <div className="min-w-0">
                      {s.tag && (
                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium ${tone.tag}`}>{s.tag}</span>
                      )}
          ...
          {vendor.whyWinsIntro?.outro && (
            <p className="mx-auto mt-10 max-w-3xl text-center text-sm leading-relaxed text-slate-600 sm:mt-12 sm:text-base">{vendor.whyWinsIntro.outro}</p>
          )}
        </div>
      </section>
        ...
        <div className="shell relative">
          <div className="border-b border-white/10 pb-5">
            <h2 className="font-display text-xl font-bold leading-tight text-white sm:text-2xl lg:text-[1.7rem]">
              Who should put{" "}
              <span className="bg-gradient-to-r from-[#28B5E1] to-[#7ad4f0] bg-clip-text text-transparent">{vendor.name}</span>{" "}
              on the shortlist
            </h2>
          </div>
      ...
      {vendor.products.length > 0 && (
        <section id="sizing" className="relative bg-slate-50 py-16 sm:py-24">
          <div className="shell">
            <div className="max-w-3xl">
              <p className="font-mono text-xs font-semibold uppercase tracking-[0.25em] text-[#1B8AC7]">Editions &amp; packaging</p>
              <h2 className="mt-4 font-display text-3xl font-bold leading-tight text-slate-900 sm:text-4xl">Tiers and editions we deploy</h2>
            ...
            <div className="mt-10 overflow-hidden rounded-2xl border border-[#0A3D6B]/40 bg-white shadow-[0_10px_40px_-12px_rgba(15,23,42,0.12)]">
              ...
              {vendor.products.map((p) => (
                <div key={p.model} className="grid grid-cols-3 gap-4 border-t border-[#0A3D6B]/20 px-5 py-4 transition-colors hover:bg-[#28B5E1]/[0.04] sm:px-7">
                  <span className="font-display text-sm font-semibold text-slate-900 sm:text-base">{p.model}</span>
      ...
      {vendor.watchOuts.length > 0 && (
        <section className="relative bg-white py-16 sm:py-24">
          <div className="shell">
            <div className="max-w-3xl">
              <p className="font-mono text-xs font-semibold uppercase tracking-[0.25em] text-amber-600">What to consider</p>
              <h2 className="mt-4 font-display text-3xl font-bold leading-tight text-slate-900 sm:text-4xl">The honest watch-outs</h2>
              <p className="mt-4 text-base leading-relaxed text-slate-600 sm:text-lg">
            ...
            <div className="mt-10 grid gap-5 lg:grid-cols-2">
              {vendor.watchOuts.map((w) => (
                <div key={w.title} className="rounded-2xl border border-amber-200 bg-amber-50/60 p-7">
                  <h3 className="font-display text-lg font-semibold text-slate-900 sm:text-xl">{w.title}</h3>
                  <p className="mt-3 text-sm leading-relaxed text-slate-700 sm:text-base">{w.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="relative bg-white py-12 sm:py-16">
          <div className="shell">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:gap-6">
              <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.25em] text-slate-400">{vendor.deploymentOptions.eyebrow ?? "Deployment Options"}</p>
      ...
      <section className="relative bg-white py-16 sm:py-24">
        <div className="shell">
          <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#0A3D6B] via-[#04101E] to-[#020617] p-8 shadow-xl shadow-[#1B8AC7]/10 sm:p-12">
            ...
            <h2 className="mt-4 font-display text-2xl font-bold leading-tight text-white sm:text-3xl md:text-4xl">Delivering {vendor.name} across the UAE</h2>
            <p className="mt-6 text-base leading-relaxed text-slate-200 sm:text-lg">{vendor.whyArtiflex}</p>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              <Link to={group.href} className="inline-flex items-center justify-center gap-2 rounded-full border border-white/15 bg-white/5 px-6 py-3 text-sm font-semibold text-white transition-all hover:border-[#28B5E1]/50 hover:bg-white/10 sm:text-base">Back to {group.label}</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative bg-slate-50 py-16 sm:py-24">
        <div className="shell">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 font-display text-3xl font-bold leading-tight text-slate-900 sm:text-4xl">{vendor.name} questions we hear from UAE buyers</h2>
          </div>
          <div className="mt-10">
      ...
      <CTASection
        ...
        description="Free assessment, vendor-neutral sizing, and a written recommendation. We will tell you when another platform is the better fit."
        primaryButton={{ text: "Request the assessment", action: "modal" }}
        secondaryButton={{ text: `Compare all ${group.label} platforms`, href: `${group.href}#vendor-comparison` }}
      />
    </>
  );
}
