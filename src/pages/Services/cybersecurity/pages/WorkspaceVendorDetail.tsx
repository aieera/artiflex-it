...
import { workspaceVendors } from "../data/workspaceVendors";
...
export default function WorkspaceVendorDetail() {
  const { slug } = useParams<{ slug: string }>();
  const vendor = slug ? workspaceVendors[slug] : undefined;
  ...
  if (!vendor) {
    return <Navigate to="/cybersecurity/workspace-protection-sse-sase" replace />;
  }

  return (
    <>
      <title>{`${vendor.name} | Workspace Protection (SSE / SASE / CASB) UAE | Artiflex IT`}</title>
      <meta name="description" content={vendor.description.slice(0, 160)} />
      <link
        rel="canonical"
        href={`https://artiflexit.com/cybersecurity/workspace/${vendor.slug}`}
      />
            ...
            >
              ...
              <div className="mt-10 flex flex-col gap-5">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-3">
                  ...
                  <Link
                    to="/cybersecurity/workspace-protection-sse-sase#vendor-matrix"
                  ...
                  <Link
                    to="/cybersecurity/workspace-protection-sse-sase#gartner-comparison"
                    className="group inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-xs font-semibold text-slate-200 transition-all hover:-translate-y-0.5 hover:border-[#28B5E1]/60 hover:bg-white/10 hover:text-white sm:text-[13px]"
                  >
      ...
      <CTASection
        ...
        description="Free Workspace Protection assessment, vendor-neutral sizing, and a written recommendation. We will tell you when another vendor is the better fit."
        primaryButton={{ text: "Request the assessment", action: "modal" }}
        secondaryButton={{
          text: "Compare all vendors",
          href: "/cybersecurity/workspace-protection-sse-sase#vendor-comparison",
        }}
      />
    </>
  );
