...
import { endpointVendors } from "../data/endpointVendors";
...
export default function EndpointVendorDetail() {
  const { slug } = useParams<{ slug: string }>();
  const vendor = slug ? endpointVendors[slug] : undefined;
  ...
  if (!vendor) {
    return <Navigate to="/cybersecurity/endpoint-security-edr-xdr" replace />;
  }

  return (
    <>
      <title>{`${vendor.name} | Endpoint Security UAE | Artiflex IT`}</title>
      <meta name="description" content={vendor.description.slice(0, 160)} />
      <link
        rel="canonical"
        href={`https://artiflexit.com/cybersecurity/endpoint/${vendor.slug}`}
      />
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#04101E] to-[#0A3D6B]">
        ...
        <div className="relative z-10 border-b border-white/5">
          <div className="shell flex w-full items-center gap-2 pt-24 pb-3 text-[11px] text-slate-400 sm:pt-28">
            ...
            <span className="text-slate-600">/</span>
            <Link
              to="/cybersecurity/endpoint-security-edr-xdr"
            ...
            >
              ...
              <div className="mt-10 flex flex-col gap-5">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-3">
                  ...
                  <Link
                    to="/cybersecurity/endpoint-security-edr-xdr#vendor-matrix"
                  ...
                  <Link
                    to="/cybersecurity/endpoint-security-edr-xdr#gartner-comparison"
                    className="group inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-xs font-semibold text-slate-200 transition-all hover:-translate-y-0.5 hover:border-[#28B5E1]/60 hover:bg-white/10 hover:text-white sm:text-[13px]"
                  >
      ...
      <section id="sizing" className="relative bg-slate-50 py-16 sm:py-24 scroll-mt-24">
        <div className="shell">
          ...
          {vendor.portfolioNote && (
            <div className="mt-8 flex items-start gap-4 rounded-2xl border border-amber-200 border-l-4 border-l-amber-500 bg-amber-50/70 p-5 sm:p-6">
              <span
                aria-hidden="true"
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-amber-500 text-base font-bold text-white"
              >
                !
              </span>
              <div>
                <h3 className="font-display text-sm font-semibold text-amber-900 sm:text-base">
                  {vendor.portfolioNote.title}
                </h3>
                <p className="mt-1.5 text-[13px] leading-relaxed text-amber-800 sm:text-sm">
                  {vendor.portfolioNote.body}
                </p>
              </div>
            </div>
          )}
            ...
            {vendor.products.map((p) => (
              ...
              >
                <span className="font-display text-sm font-semibold text-slate-900 sm:text-base">
                  ...
                  {p.tag && (
                    <span
                      className={`ml-2 inline-block whitespace-nowrap rounded-md px-1.5 py-0.5 align-middle text-[10px] font-bold uppercase tracking-wide ${
                        p.tag.tone === "eol"
      ...
      <CTASection
        ...
        description="Free endpoint assessment, vendor-neutral sizing, and a written recommendation. We will tell you when another vendor is the better fit."
        primaryButton={{ text: "Request the assessment", action: "modal" }}
        secondaryButton={{
          text: "Compare all vendors",
          href: "/cybersecurity/endpoint-security-edr-xdr#vendor-matrix",
        }}
      />
    </>
  );
