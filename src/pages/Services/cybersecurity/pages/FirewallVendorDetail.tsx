...
import { firewallVendors } from "../data/firewallVendors";

export default function FirewallVendorDetail() {
  const { slug } = useParams<{ slug: string }>();
  const vendor = slug ? firewallVendors[slug] : undefined;
  ...
  if (!vendor) {
    return <Navigate to="/cybersecurity/firewalls-network-security" replace />;
  }

  // Build a safe meta description, break on word boundary if the source
  // is longer than the snippet limit, so Google does not show a half-cut
  // sentence. Falls back to plain slice if the description is already short.
  ...
  const pageUrl = `https://artiflexit.com/cybersecurity/firewalls/${vendor.slug}`;
  const pageTitle = `${vendor.name} | Firewall Solutions UAE | Artiflex IT`;
  const ogImage = "https://artiflexit.com/og/firewalls-network-security.png";
  ...
  return (
    <>
      ...
      {/* JSON-LD: Product schema, vendor's firewall as a product entry */}
        ...
        {JSON.stringify({
          ...
          itemListElement: [
            ...
            { "@type": "ListItem", position: 2, name: "Cybersecurity", item: "https://artiflexit.com/cybersecurity" },
            {
              ...
              item: "https://artiflexit.com/cybersecurity/firewalls-network-security",
            },
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#04101E] to-[#0A3D6B]">
        ...
        <div className="relative z-10 border-b border-white/5">
          <div className="shell flex w-full items-center gap-2 pt-24 pb-3 text-[11px] text-slate-400 sm:pt-28">
            ...
            <span className="text-slate-600">/</span>
            <Link
              to="/cybersecurity/firewalls-network-security"
            ...
            >
              ...
              <div className="mt-10 flex flex-col gap-5">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-3">
                  ...
                  <Link
                    to="/cybersecurity/firewalls-network-security#vendor-matrix"
                  ...
                  <Link
                    to="/cybersecurity/firewalls-network-security#gartner-comparison"
                    className="group inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-xs font-semibold text-slate-200 transition-all hover:-translate-y-0.5 hover:border-[#28B5E1]/60 hover:bg-white/10 hover:text-white sm:text-[13px]"
                  >
      ...
      {vendor.keyStats.length > 0 && (
        <section className="relative bg-slate-50 py-12 sm:py-16">
          <div className="mx-auto max-w-4xl px-8 sm:px-6">
            <div
              className={`grid gap-4 sm:grid-cols-2 ${vendor.keyStats.some((s) => s.wide)
                  ? "lg:grid-cols-3"
                  : "lg:grid-cols-4"
                }`}
            >
              {vendor.keyStats.map((s) => (
                <div
                  key={s.label}
                  className={`rounded-2xl border border-slate-200 bg-white p-5 shadow-sm ${s.wide ? "sm:col-span-2 lg:col-span-3" : ""
                    }`}
                >
          ...
          <div className="border-b border-white/10 pb-5">
            <h2 className="font-display text-xl font-bold leading-tight text-white sm:text-2xl lg:text-[1.7rem]">
              ...
              <span className="bg-gradient-to-r from-[#28B5E1] to-[#7ad4f0] bg-clip-text text-transparent">
                {vendor.name.replace(" XGS", " Firewall")}
              </span>{" "}
              on the shortlist
            </h2>
          </div>
        ...
        <div className="shell">
          <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#0A3D6B] via-[#04101E] to-[#020617] p-8 shadow-xl shadow-[#1B8AC7]/10 sm:p-12">
            ...
            <h2 className="mt-4 font-display text-2xl font-bold leading-tight text-white sm:text-3xl md:text-4xl">
              Delivering {vendor.name.replace(" XGS", " Firewall")} across the UAE
            </h2>
      ...
      <CTASection
        ...
        description="Free network assessment, vendor-neutral sizing, and a written recommendation. We will tell you when another vendor is the better fit."
        primaryButton={{ text: "Request the assessment", action: "modal" }}
        secondaryButton={{
          text: "Compare all vendors",
          href: "/cybersecurity/firewalls-network-security#vendor-matrix",
        }}
      />
    </>
  );
