...
import { vmVendors } from "../data/vmVendors";
...
export default function VmVendorDetail() {
  const { slug } = useParams<{ slug: string }>();
  const vendor = slug ? vmVendors[slug] : undefined;
  ...
  if (!vendor) {
    return <Navigate to="/cybersecurity/vulnerability-management" replace />;
  }

  return (
    <>
      <title>{`${vendor.name} | Vulnerability Management UAE | Artiflex IT`}</title>
      <meta name="description" content={vendor.description.slice(0, 160)} />
      <link
        rel="canonical"
        href={`https://artiflexit.com/cybersecurity/vulnerability-management/${vendor.slug}`}
      />
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#04101E] to-[#0A3D6B]">
        ...
        <div className="relative z-10 border-b border-white/5">
          <div className="shell flex w-full items-center gap-2 pt-24 pb-3 text-[11px] text-slate-400 sm:pt-28">
            ...
            <span className="text-slate-600">/</span>
            <Link
              to="/cybersecurity/vulnerability-management"
            ...
            >
              ...
              <div className="mt-10 flex flex-col gap-5">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-3">
                  ...
                  <Link
                    to="/cybersecurity/vulnerability-management#vendor-matrix"
                  ...
                  <Link
                    to="/cybersecurity/vulnerability-management#gartner-comparison"
                    className="group inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-xs font-semibold text-slate-200 transition-all hover:-translate-y-0.5 hover:border-[#28B5E1]/60 hover:bg-white/10 hover:text-white sm:text-[13px]"
                  >
      ...
      {/* EXTRA VENDOR SECTIONS */}
      {vendor.extraSections?.map((sec, sIdx) => (
        <section
          key={sec.title}
          className={`relative py-16 sm:py-24 ${sIdx % 2 === 0 ? "bg-white" : "bg-slate-50"}`}
        >
          <div className="shell">
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {sec.items.map((it) => (
                <div
                  key={it.title}
                  className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_6px_24px_-14px_rgba(15,23,42,0.15)] transition-all duration-300 hover:-translate-y-0.5 hover:border-[#28B5E1]/40 hover:shadow-[0_14px_40px_-16px_rgba(40,181,225,0.3)]"
                >
      ...
      <CTASection
        ...
        description="Free Vulnerability Management assessment, vendor-neutral sizing, and a written recommendation. We will tell you when another vendor is the better fit."
        primaryButton={{ text: "Request the assessment", action: "modal" }}
        secondaryButton={{
          text: "Compare all vendors",
          href: "/cybersecurity/vulnerability-management#vendor-comparison",
        }}
      />
    </>
  );
