...
export default function InfrastructurePage() {
  return (
    <>
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <Card variant="glass" hover={false} className="p-8">
              <p className="text-slate-400 leading-relaxed mb-4">The average cost of IT downtime is <span className="text-white font-semibold">$5,600 per minute</span>. For a mid-market business, a four-hour outage can cost more than an entire year of proactive infrastructure management.</p>
              <p className="text-slate-400 leading-relaxed">73% of unplanned downtime in the region is caused by aging infrastructure — equipment running past its lifecycle, firmware that hasn't been updated in years, and networks that were designed for half the current workload. This isn't bad luck. It's predictable failure from deferred investment.</p>
            </Card>
          </div>
        </div>
      </section>
            ...
            {coreServices.map((service) => (
              <Card key={service.title} variant="service" className="p-6">
                ...
                <ul className="space-y-2 mb-4">
                  {service.features.map((f) => (<li key={f} className="flex items-start gap-2"><CheckIcon className="w-4 h-4 text-brand-blue shrink-0 mt-0.5" /><span className="text-sm text-slate-300">{f}</span></li>))}
                </ul>
                <div className="rounded-lg bg-brand-blue/10 border border-brand-blue/30 p-3"><p className="text-sm font-medium text-white">{service.outcome}</p></div>
              </Card>
            ))}
          </div>
        </div>
      ...
      <CTASection title="Build Infrastructure That Doesn't Keep You Up at Night" description="Get an infrastructure assessment that identifies aging equipment, capacity constraints, and single points of failure — with a prioritized upgrade roadmap." primaryButton={{ text: "Request Infrastructure Review", action: "modal" }} secondaryButton={{ text: "View All Services", href: "/services" }} />
    </>
  );
}
