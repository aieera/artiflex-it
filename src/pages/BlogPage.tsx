...
export default function BlogPage() {
      ...
      <section className="relative py-16 bg-navy-deep sm:py-24">
        <div className="mx-auto max-w-7xl px-5 sm:px-6">
          ...
          <div className="grid gap-6 sm:gap-8 lg:grid-cols-[1.2fr_1fr] items-stretch">
            ...
            <img src="/public/cybermain.jpeg"/>
              <div className="absolute inset-0 bg-grid-pattern opacity-30" />
              <div className="absolute top-2 left-6"><span className="inline-flex rounded-full bg-brand-blue/20 border border-brand-blue/40 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-brand-blue">{featuredArticle.tag}</span></div>
  ...
  <img
    ...
    blur-[6px] scale-110 
    group-hover:blur-none group-hover:scale-100"
  />

  {/* Gradient overlay */}
  <div
    className={`absolute inset-0 bg-gradient-to-br ${article.gradient}
    opacity-90 group-hover:opacity-0 transition-all duration-500`}
  />
  ...
  <div className="absolute top-4 left-4">
    <span className={`inline-flex rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-widest ${article.tagColor}`}>
      {article.tag}
    </span>
  </div>
</div>
