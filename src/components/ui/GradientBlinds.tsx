...
const prepStops = (stops?: string[]) => {
  const base = (stops?.length ? stops : ["#0A3D6B", "#1B8AC7", "#28B5E1"]).slice(0, MAX_COLORS);
  if (base.length === 1) base.push(base[0]);
  while (base.length < MAX_COLORS) base.push(base[base.length - 1]);
  const arr = base.map(hexToRGB);
  return { arr, count: Math.max(2, Math.min(MAX_COLORS, stops?.length ?? 3)) };
};
...
export default function GradientBlinds({
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      mouseTarget.current = [(e.clientX - rect.left) * s, (rect.height - (e.clientY - rect.top)) * s];
    };
    ...
    const loop = (t: number) => {
      ...
      if (mouseDampening > 0) {
        ...
        cur[0] += (mouseTarget.current[0] - cur[0]) * f;
        cur[1] += (mouseTarget.current[1] - cur[1]) * f;
      }
  ...
  }, [gradientColors, angle, noise, blindCount, blindMinWidth, mouseDampening, mirrorGradient, spotlightRadius, spotlightSoftness, spotlightOpacity, distortAmount, shineDirection]);
